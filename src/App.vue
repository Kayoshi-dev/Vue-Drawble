<script setup lang="ts">
  import { VueDrawble } from './components';
  import { ref } from 'vue';
...
<template>
  <h1 style="padding-bottom: 3rem;">Vue Drawble Example</h1>

  <VueDrawble style="padding-bottom: 4rem;" :rows="rows" :columns="['id', 'command cli', 'description']">
  ...
  <VueDrawble :rows="rows" :columns="{id: 'ID', name: 'Name', 'command cli': 'Command to execute'}">
    <template #commandCli="{ 'command cli': commandCli  }">{{ commandCli }}</template>
